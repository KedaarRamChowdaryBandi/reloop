...
with col1:
    ...
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">Total Material Listed</h4>
        <h2 style="color: #00E676; margin: 10px 0;">{total_listed}</h2>
        <p style="color: #666; font-size: 11px;">Material lots listed</p>
    </div>
...
with col2:
    st.markdown(f"""
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">Lots Sold</h4>
        <h2 style="color: #00E676; margin: 10px 0;">{total_sold}</h2>
        <p style="color: #666; font-size: 11px;">Transferred to circular chain</p>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    st.markdown(f"""
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">Total Earnings</h4>
        <h2 style="color: #00E676; margin: 10px 0;">₹{total_earnings:,.2f}</h2>
        <p style="color: #666; font-size: 11px;">Revenue generated</p>
    </div>
    """, unsafe_allow_html=True)
...
with col4:
    st.markdown(f"""
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">CO₂ Diverted</h4>
        <h2 style="color: #00E676; margin: 10px 0;">{total_co2:,.2f} t</h2>
        <p style="color: #666; font-size: 11px;">Offset savings achieved</p>
    </div>
    """, unsafe_allow_html=True)
...
if listings:
    ...
    with col_chart1:
        ...
        st.bar_chart(mat_counts.set_index("Material"), y="Count", color="#00E676")
        
    with col_chart2:
        st.markdown("##### Listing Status")
        status_counts = df["status"].value_counts().reset_index()
        status_counts.columns = ["Status", "Count"]
        st.bar_chart(status_counts.set_index("Status"), y="Count", color="#123524")
else:
