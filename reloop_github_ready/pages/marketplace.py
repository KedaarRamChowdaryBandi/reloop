...
if not listings:
    ...
    for l in listings:
        ...
        if is_verified:
            ...
            badge = '<span class="badge-pending" style="color: #aaa; border-color: #aaa; background-color: transparent;">Unverified Supplier</span>'
            
        trust_score = l["trust_score"] if l["trust_score"] is not None else 5.0
        trust_color = "#00E676" if trust_score >= 8.0 else "#ffc107" if trust_score >= 5.0 else "#ff5252"
        ...
        with col_card:
            ...
            <div class="reloop-card">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                    <h3 style="margin: 0; color: #00E676;">{l['material'].title()}</h3>
                    <div>{badge}</div>
                </div>
                <p style="color: #ddd; font-style: italic; font-size: 13px; margin-bottom: 12px;">{l['description'] or 'No description provided.'}</p>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; font-size: 13px; color: #bbb;">
