...
with col1:
    ...
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">Materials Purchased</h4>
        <h2 style="color: #00E676; margin: 10px 0;">{total_procured:,.1f} kg</h2>
        <p style="color: #666; font-size: 11px;">Procured waste byproducts</p>
    </div>
...
with col2:
    st.markdown(f"""
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">Total Spent</h4>
        <h2 style="color: #00E676; margin: 10px 0;">₹{total_spent:,.2f}</h2>
        <p style="color: #666; font-size: 11px;">Circularity investments</p>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    st.markdown(f"""
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">Total CO₂ Saved</h4>
        <h2 style="color: #00E676; margin: 10px 0;">{total_co2:,.2f} t</h2>
        <p style="color: #666; font-size: 11px;">Carbon footprint reduction</p>
    </div>
    """, unsafe_allow_html=True)
...
with col4:
    st.markdown(f"""
    <div class="reloop-card" style="text-align: center;">
        <h4 style="color: #888; font-size: 14px;">Total Transactions</h4>
        <h2 style="color: #00E676; margin: 10px 0;">{order_count}</h2>
        <p style="color: #666; font-size: 11px;">Completed orders</p>
    </div>
    """, unsafe_allow_html=True)
...
if orders:
    ...
    with c_col1:
        ...
        st.line_chart(daily_spend.set_index("Date"), y="total_price", color="#00E676")
        
    with c_col2:
        st.markdown("##### Material Procurement Breakdown")
        mat_breakdown = df.groupby("material")["quantity"].sum().reset_index()
        st.bar_chart(mat_breakdown.set_index("material"), y="quantity", color="#123524")
else:
