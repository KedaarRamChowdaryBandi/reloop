...
if submit_btn:
    if not req_material:
        ...
        if not matches:
            ...
            with col_award1:
                st.markdown(f"""
                <div class="reloop-card" style="border-color: #00E676; height: 260px;">
                    <h3 style="color: #00E676; font-size: 16px;">🥇 Best Match</h3>
                    <h4 style="margin: 10px 0; color: white;">{best_match['material'].title()}</h4>
                    <p style="font-size: 13px; color: #bbb; min-height: 45px;">Seller: {best_match['supplier_name']}<br>Location: {best_match['location']}</p>
                    <h2 style="color: #00E676; margin: 10px 0; font-size: 24px;">{best_match['match_score']:.1f}%</h2>
                    <p style="font-size: 11px; color: #888;">Optimized match score</p>
                </div>
            ...
            with col_award2:
                st.markdown(f"""
                <div class="reloop-card" style="border-color: #29b6f6; height: 260px;">
                    <h3 style="color: #29b6f6; font-size: 16px;">📍 Nearest</h3>
                    <h4 style="margin: 10px 0; color: white;">{nearest['material'].title()}</h4>
                    <p style="font-size: 13px; color: #bbb; min-height: 45px;">Seller: {nearest['supplier_name']}<br>Location: {nearest['location']}</p>
                    <h2 style="color: #29b6f6; margin: 10px 0; font-size: 24px;">{nearest['distance_km']:.1f} km</h2>
                    <p style="font-size: 11px; color: #888;">Shortest transport route</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col_award3:
                st.markdown(f"""
                <div class="reloop-card" style="border-color: #ff9100; height: 260px;">
                    <h3 style="color: #ff9100; font-size: 16px;">💰 Cheapest</h3>
                    <h4 style="margin: 10px 0; color: white;">{cheapest['material'].title()}</h4>
                    <p style="font-size: 13px; color: #bbb; min-height: 45px;">Seller: {cheapest['supplier_name']}<br>Location: {cheapest['location']}</p>
                    <h2 style="color: #ff9100; margin: 10px 0; font-size: 24px;">₹{cheapest['price_per_kg']:.1f}/kg</h2>
                    <p style="font-size: 11px; color: #888;">Lowest cost per unit</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col_award4:
                st.markdown(f"""
                <div class="reloop-card" style="border-color: #ec407a; height: 260px;">
                    <h3 style="color: #ec407a; font-size: 16px;">⭐ Highest Trust</h3>
                    <h4 style="margin: 10px 0; color: white;">{highest_trust['material'].title()}</h4>
                    <p style="font-size: 13px; color: #bbb; min-height: 45px;">Seller: {highest_trust['supplier_name']}<br>Trust Score: {highest_trust['trust_score'] or 5.0:.1f}</p>
                    <h2 style="color: #ec407a; margin: 10px 0; font-size: 24px;">{highest_trust['trust_score'] or 5.0:.1f}/10</h2>
                    <p style="font-size: 11px; color: #888;">Vetted credentials index</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            for m in sorted_matches:
                ...
                <div class="reloop-card">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                        <h4 style="margin: 0; color: white;">♻️ {m['material'].title()}</h4>
                        <div style="font-size: 18px; font-weight: bold; color: {score_color};">Match Score: {m['match_score']:.1f}%</div>
                    </div>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; font-size: 13px; color: #bbb; margin-bottom: 15px;">
